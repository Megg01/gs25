...
function AppAll(alldatas, targetAllElement) {
  let retAllVal = "";

  for (const data of alldatas) {
    let productAllObj = new ProductAllCardItem(
      data.src,
      data.alt,
      data.name,
      data.price
  ...
  document
    ...
    .insertAdjacentHTML("beforeend", retAllVal);
}

const ProductAllCardData = [
  {
    src: "img/product-img/product-hotdog1.webp",
    alt: "hotdog",
    name: "Франкфурт хотдог",
...
window.onload = () => {
  ...
  AppAll(ProductAllCardData, "all-products");
};
